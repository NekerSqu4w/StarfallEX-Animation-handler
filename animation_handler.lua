...
if CLIENT then
    ...
    //Used to setup your own objects for animation
    local objects_type = {
        rect = {
        ...
        rounded_box = {
            func = function(data,real_ratio,direct_ratio)
                render.setColor(Color(data.r,data.g,data.b,data.a))
                render.drawRoundedBox(data.radius,data.x,data.y,data.width,data.height)
            end
        }
    }
    
    //Set the new animation
    animation_plus.set_animation(animation,objects_type)
